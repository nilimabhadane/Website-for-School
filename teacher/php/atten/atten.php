...
<?php
	...
	function insertEmployee($params) {
		 ...
		 extract($params);
		   ...
		   $sql = "INSERT INTO `school`.`attendance` (`ID`, `adate`, `mnth`, `tname`, `rno`, `sname`, `nofp`, `nofa`, `holi`) VALUES (NULL, '$adate', '$mnth', '$tname', '$rno', '$sname', '$nofp', '$nofa', '$holi')";
...
if(empty($data)){
  $data=array();
	...
	function updateEmployee($params) {
		...
		$sql = "UPDATE `attendance` SET `adate` = '" . $params["adate"]."',`mnth` = '" . $params["mnth"]."',`tname` = '" . $params["tname"]."',`rno` = '" . $params["rno"]."',`sname` = '" . $params["sname"]."',`nofp` = '" . $params["nofp"]."',`nofa` = '" . $params["nofa"]."',`holi` = '" . $params["holi"]."' WHERE ID='".$params["edit_ID"]."'";
